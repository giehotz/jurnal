...
<div class="container-fluid">
    ...
    <!-- Navigation Tabs -->
    <div style="display: flex; gap: 10px; border-bottom: 2px solid #f0f0f0; flex-wrap: wrap;">
        <a href="<?= base_url('guru/profile') ?>" style="flex: 1; max-width: 200px; text-align: center; padding: 10px 5px; color: #999; text-decoration: none; font-weight: 500; border-bottom: 3px solid transparent; transition: all 0.3s; margin-bottom: -2px;">
            <i class="fas fa-user-circle"></i> Profil
        </a>
        <a href="<?= base_url('guru/profile/edit') ?>" style="flex: 1; max-width: 200px; text-align: center; padding: 10px 5px; color: #667eea; text-decoration: none; font-weight: 500; border-bottom: 3px solid #667eea; margin-bottom: -2px;">
            <i class="fas fa-edit"></i> Edit Data
        </a>
        <a href="<?= base_url('guru/profile/change-password') ?>" style="flex: 1; max-width: 200px; text-align: center; padding: 10px 5px; color: #999; text-decoration: none; font-weight: 500; border-bottom: 3px solid transparent; transition: all 0.3s; margin-bottom: -2px;">
            ...
            <div class="form-section">
                ...
                <div class="picture-preview-box banner-preview <?= (!empty($user['banner'])) ? 'has-image' : '' ?>" style="padding:0;">
                    <?php if (!empty($user['banner'])): ?>
                        <img id="bannerPreviewImage" src="<?= base_url('uploads/profile_banners/' . $user['banner']) ?>?v=<?= time() ?>" alt="Banner Preview" style="width:100%;height:auto;border-radius:8px;max-height:240px;object-fit:cover;">
                    <?php else: ?>
...
<script>
    ...
    document.getElementById('bannerRotateCW').addEventListener('click', function(e) {
        e.preventDefault();
        if (bannerCropper) bannerCropper.rotate(45);
    });
    document.getElementById('bannerRotateCCW').addEventListener('click', function(e) {
        e.preventDefault();
        if (bannerCropper) bannerCropper.rotate(-45);
    });
    document.getElementById('bannerResetCrop').addEventListener('click', function(e) {
        e.preventDefault();
        if (bannerCropper) bannerCropper.reset();
    });
